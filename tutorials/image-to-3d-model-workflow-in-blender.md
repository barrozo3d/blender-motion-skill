...
- [Step by Step: Image File to 3D Geometry | Blender Secrets](step-by-step-image-file-to-3d-geometry-blender-secrets.md) — shares modelling, procedural; uses the identical image-to-Grease-Pencil-to-mesh trace pipeline from this video's "Path A," applied there to cut a logo into a curved surface via Knife Project rather than building a full blockout.
- [This technique lets you make Hard Surface models easily](this-technique-lets-you-make-hard-surface-models-easily.md) — shares procedural, displacement, advanced; the dedicated tutorial on building a tiling displacement map from scratch, the exact technique this video's hard-surface detailing pass applies.
