...
- [Image to 3D model workflow in Blender](image-to-3d-model-workflow-in-blender.md) — shares modelling, organic, procedural, intermediate; that video's "Path B" box-modeling technique from a cube is the identical double-Subdivision-modifier method taught here, alongside an alternative trace-image-to-mesh path.
