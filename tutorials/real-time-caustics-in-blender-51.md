...
author: Extra 3d
ingested: 2026-06-25
blender_version: "Blender 5.1.2 -- observed in frame_000, frame_001, frame_002, frame_003"
...
3. **Shadow pass-through:** Add Mix Shader after glass. Add Transparent Shader into Shader slot 2. Add Light Path → Shadow Ray → Mix Shader Factor. **Confirmed exactly as written** [frame_002]: the material (named `Glass`) resolves to Surface = `Mix Shader`, `Factor` = **`Light Path | Is …`**, Shader 1 = `Principled BSDF`, Shader 2 = `Transparent B…`. The Transparent shader's colour is open on the picker at `Hex` **FFFFFFFF** (Hue 0.000, Saturation 0.000, Value 1.000) — i.e. still pure white, the blank caustic canvas before any pattern is wired in. The Principled beneath it runs `Roughness` **0.000** with `Transmission → Weight` as the highlighted socket.
...
7. **Water caustics:** ✅ **This step survives the frame check intact** [frame_003] — two `Voronoi Texture` nodes, both **4D**, **Smooth F1**, `Randomness` **0.900**, with `Smoothness` **0.400** on the first and **0.200** on the second, feeding a Mix Color in **Difference** mode at `Factor` **1.000**. The fields it did not record: distance metric **Euclidean**, `Normalize` off, `W` 0.000, `Scale` **5.000**, `Detail` 0.000, `Roughness` 0.500, `Lacunarity` 2.000 on both, and `Clamp Factor` **✓** on the Difference node. The material is named **`Water Caustics`**, and at this stage the `Mapping` node is still neutral — Location 0,0,0 / Rotation 0° / Scale 1,1,1 — so the Key Step 11 transform has not been applied yet. Texture Coordinate Object + shared Scale Value node.
8. **Animate caustics:** Add Value node → keyframe from 0 to large number → plug into both Voronoi W. Add Multiply Math (speed control).
...
12. **Noise fringe layer:** `Gradient Texture` — type **Radial**, which the entry never recorded — into `Noise Texture` [frame_004]. The Noise reads **3D**, **fBM**, `Normalize` **✓**, `Scale` **1.000** (low ✓), `Detail` **15.000** (max ✓), `Roughness` **1.000** (max ✓), `Lacunarity` 2.000 — but ⚠️ **`Distortion` reads 0.000, not 200**. Three of the four recorded values check out and the fourth does not appear in any frame. Color Ramp Constant (3 fringes, decrease white value). Color Ramp Ease (move black handle right). Mix Color of both Color Ramps. Multiply ×2.
13. **Final color:** Mix Color (Multiply, Factor 1) → control caustic color from this value.

### Nodes / Settings
- **Voronoi** Texture (the entry said "Voronoid" throughout — the node is `Voronoi Texture` [frame_001, frame_003]): 4D; Smooth F1; **Euclidean**; Normalize off; W 0.000; Scale **5.000**; Detail 0.000; Roughness 0.500; Lacunarity 2.000; Smoothness **0.400** / **0.200**; Randomness **0.900** [frame_003]
- Mix Color (water): **Difference**, `Factor` 1.000, `Clamp Factor` ✓, A/B = the two Voronoi textures [frame_003]
- An earlier, exploratory Voronoi at 3:30 is a *different* configuration — **3D / F1 / Euclidean**, Scale **12.500**, Detail 15.000, Roughness 0.500, Lacunarity 2.000, Randomness 1.000, wired straight into Material Output to preview it [frame_001]. Not the water-caustics setup; do not conflate the two
- Mix Color (water): Difference mode, Factor 1.0
- `Gradient Texture` type **Radial** → `Noise Texture`: **3D**, **fBM**, `Normalize` ✓, Scale **1.000**, Detail **15.000**, Roughness **1.000**, Lacunarity 2.000, **Distortion 0.000** [frame_004]. The recorded "distortion 200" appears in no frame
...
### Blender Version
**Blender 5.1.2** — status bar, four frames [frame_000, frame_001, frame_002, frame_003]. The same build as `how-to-render-faster-in-blender-cycles.md`, which is the same author; that entry had been filed under "Blender 4.x".
...
| | |
|---|---|
| **Confirmed** | Key Step 3 is exact — `Mix Shader` with `Factor` = `Light Path | Is …`, Principled in slot 1, Transparent in slot 2, on a material named `Glass` [frame_002]. **Key Step 7 is exact too**: both Voronoi textures at 4D / Smooth F1 / Randomness 0.900, Smoothness 0.400 and 0.200, mixed in `Difference` at Factor 1.000 [frame_003]. Two consecutive steps needing no correction is a first for this batch. |
| **Corrected** | `blender_version` `Blender 5.1` → **5.1.2**. The texture node is **`Voronoi`**, not "Voronoid" — a transcription artifact that had propagated into the Summary, two Key Steps and the Nodes/Settings list. Key Step 12's `Distortion 200` is **0.000** in the only frame showing that node [frame_004]. |
| **Added** | every unrecorded field on the two Voronoi nodes — Euclidean, Normalize off, W 0.000, Scale 5.000, Detail 0.000, Roughness 0.500, Lacunarity 2.000 — plus `Clamp Factor` on the Difference node [frame_003]; the `Gradient Texture` **Radial** type and the Noise node's fBM / Normalize settings [frame_004]; the material names `Glass` and **`Water Caustics`**; and the scene's growth from `Camera / Plane / Sphere / Sun` at 2:00 to a second and third plane plus a **`Spot`** light by 7:20 [frame_000, frame_003]. |
| **Flagged as unverified** | the Cycles + GPU Compute engine setting, the label-exclusion mask (Key Step 6), the W-animation Value node and its speed Multiply (Key Step 8), the two softening Color Ramps, and the whole advanced chain of Key Steps 11–13 — Normalize/Add 0.5, the Z = −1 and 0.8/0.8/1.4 Mapping, Screen at 0.7, the Constant and Ease Color Ramps. The `Mapping` node visible at 7:20 is still at its defaults, so the transform had not been applied when the set was captured. |

ℹ️ **Two Voronoi configurations exist in this tutorial and only one is the
technique.** `frame_001` (3:30) catches an exploratory node — **3D / F1**, Scale
12.500 — plugged straight into Material Output so its raw output can be seen on
the sphere. `frame_003` (7:20) catches the real one: 4D / Smooth F1, Scale
5.000, paired and differenced. Both are legitimately "a Voronoi texture in this
tutorial", and reading the wrong one would have contradicted a step that is in
fact correct. Where a presenter auditions a node before committing to it,
**the earlier frame documents the audition, not the method.**
