...
- **Summary:** Designs a credit card texture in Figma (860×540px with safe areas, typography, chip, Visa logo) then imports it into Blender for a SaaS/FinTech product animation with gradient and metallic material transitions. Covers Figma layout grids, pen tool chip creation, and PNG export for use as Image Texture nodes in Blender.
...
- **Summary:** Teaches the four core fundamentals of Blender lighting — positioning, fall-off, size, and color — using a sci-fi crate as a practical example. Covers key/fill light setup, shadow importance for form reading, light radius for shadow softness, and color contrast for mood.
